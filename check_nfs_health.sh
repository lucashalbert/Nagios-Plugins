...
# 1.1           | 01.15.2019    | Lucas Halbert     | AddNFS client performance data
# =========================================================================================
...
NFS_CLIENT_SERVICES=${NFS_CLIENT_SERVICES:="portmapper nlockmgr"}
...
# -----------------------------------------------------------------------------------------
# NFS Client Statistics (Lucas Halbert - 01.15.2019)
# -----------------------------------------------------------------------------------------
function COLLECT_NFS_CLIENT_STATS() {
	local mountpoint=$1
	NFSIOSTAT_OUTPUT=$(nfsiostat $mountpoint | tail -n 6)
	TOTAL_OPS_S=$(echo "$NFSIOSTAT_OUTPUT" | head -2 | tail -1 | awk '{print $1}')
	RPC_BKLOG=$(echo "$NFSIOSTAT_OUTPUT" | head -2 | tail -1 | awk '{print $2}')
	READ_OPS_S=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $1}')
	READ_KB_S=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $2}')
	READ_KB_OP=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $3}')
	READ_TOTAL_RETRANS=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $4}')
	READ_PERCENT_RETRANS=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $5}' | sed -e 's/(//' -e 's/)//')
	READ_AVG_RTT=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $6}')
	READ_AVG_EXE=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "read:" | tail -1 | awk '{print $7}')
	WRITE_OPS_S=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $1}')
	WRITE_KB_S=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $2}')
	WRITE_KB_OP=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $3}')
	WRITE_TOTAL_RETRANS=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $4}')
	WRITE_PERCENT_RETRANS=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $5}' | sed -e 's/(//' -e 's/)//')
	WRITE_AVG_RTT=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $6}')
	WRITE_AVG_EXE=$(echo "$NFSIOSTAT_OUTPUT" | grep -A1 "write:" | tail -1 | awk '{print $7}')
}
	...
	PROC_BG_NFSCHECK=`ps -ef | grep "/tmp/nfs_health_monitor $i" | grep -v grep | wc -l`
		...
		case $NFS_SIDE in
        		server) echo "NFS CRITICAL : NFS server services ${NFS_SERVICES[@]} running. Stale NFS mountpoint $i. NFS exports ${NFS_EXPORTS[@]} are healthy | NFS Perfdata"
                		exit $STATE_CRITICAL;;
        		client) COLLECT_NFS_CLIENT_STATS ${i}
				#echo "NFS CRITICAL : NFS server services ${NFS_SERVICES[@]} running. Stale NFS mountpoint $i. NFS exports ${NFS_EXPORTS[@]} are healthy | NFS Perfdata"
                # Add performance data to output (Lucas Halbert - 01.15.2019)
				echo "NFS CRITICAL : NFS client services ${NFS_SERVICES[@]} running. Stale NFS mountpoint $i. NFS exports ${NFS_EXPORTS[@]} are healthy | ops/s=${TOTAL_OPS_S};;;; RPC_BKLOG=${RPC_BKLOG};75;100;; read_ops/s=${READ_OPS_S};;;; read_kb/s=${READ_KB_S};;;; read_kb/op=${READ_KB_OP};;;; total_read_retrans=${READ_TOTAL_RETRANS};;;; percent_read_retrans=${READ_PERCENT_RETRANS};10;15;0;100 read_avg_rtt=${READ_AVG_RTT}ms;2000;5000;; read_avg_exe=${READ_AVG_EXE}ms;2000;5000;; write_ops/s=${WRITE_OPS_S};;;; write_kb/s=${WRITE_KB_S};;;; write_kb/op=${WRITE_KB_OP};;;; total_write_retrans=${WRITE_TOTAL_RETRANS};;;; percent_write_retrans=${WRITE_PERCENT_RETRANS};10;15;0;100 write_avg_rtt=${WRITE_AVG_RTT}ms;2000;5000;; write_avg_exe=${WRITE_AVG_EXE}ms;2000;5000;;"
                		exit $STATE_CRITICAL;;
        	esac
        #elif [ ! -d $i ]; then
...
if [ -n "$PROC_NFSCHECK" ]; then
	case $NFS_SIDE in
			...
			exit $STATE_CRITICAL;;
                client) COLLECT_NFS_CLIENT_STATS ${i}
			#echo "NFS CRITICAL : NFS client services ${NFS_SERVICES[@]} running. Stale NFS mountpoint $i | NFS perfdata"
            # Add performance data to output (Lucas Halbert - 01.15.2019)
			echo "NFS CRITICAL : NFS client services ${NFS_SERVICES[@]} running. Stale NFS mountpoint $i healthy | ops/s=${TOTAL_OPS_S};;;; RPC_BKLOG=${RPC_BKLOG};75;100;; read_ops/s=${READ_OPS_S};;;; read_kb/s=${READ_KB_S};;;; read_kb/op=${READ_KB_OP};;;; total_read_retrans=${READ_TOTAL_RETRANS};;;; percent_read_retrans=${READ_PERCENT_RETRANS};10;15;0;100 read_avg_rtt=${READ_AVG_RTT}ms;2000;5000;; read_avg_exe=${READ_AVG_EXE}ms;2000;5000;; write_ops/s=${WRITE_OPS_S};;;; write_kb/s=${WRITE_KB_S};;;; write_kb/op=${WRITE_KB_OP};;;; total_write_retrans=${WRITE_TOTAL_RETRANS};;;; percent_write_retrans=${WRITE_PERCENT_RETRANS};10;15;0;100 write_avg_rtt=${WRITE_AVG_RTT}ms;2000;5000;; write_avg_exe=${WRITE_AVG_EXE}ms;2000;5000;;"
                        kill -9 $PROC_NFSCHECK
			exit $STATE_CRITICAL;;
        esac
	...
	case $NFS_SIDE in
                server) echo "NFS OK : NFS server services ${NFS_SERVICES[@]} running. NFS mountpoint "${NFS_MOUNTS[@]}" healthy. NFS exports ${NFS_EXPORTS[@]} healthy | NFS perfdata"
                        exit $STATE_OK;;
                client) COLLECT_NFS_CLIENT_STATS ${i}
			#echo "NFS OK : NFS client services ${NFS_SERVICES[@]} running. NFS mountpoint "${NFS_MOUNTS[@]}" healthy | NFS perfdata"
            # Add performance data to output (Lucas Halbert - 01.15.2019)
			echo "NFS OK : NFS client services ${NFS_SERVICES[@]} running. NFS mountpoint "${NFS_MOUNTS[@]}" healthy | ops/s=${TOTAL_OPS_S};;;; RPC_BKLOG=${RPC_BKLOG};75;100;; read_ops/s=${READ_OPS_S};;;; read_kb/s=${READ_KB_S};;;; read_kb/op=${READ_KB_OP};;;; total_read_retrans=${READ_TOTAL_RETRANS};;;; percent_read_retrans=${READ_PERCENT_RETRANS};10;15;0;100 read_avg_rtt=${READ_AVG_RTT}ms;2000;5000;; read_avg_exe=${READ_AVG_EXE}ms;2000;5000;; write_ops/s=${WRITE_OPS_S};;;; write_kb/s=${WRITE_KB_S};;;; write_kb/op=${WRITE_KB_OP};;;; total_write_retrans=${WRITE_TOTAL_RETRANS};;;; percent_write_retrans=${WRITE_PERCENT_RETRANS};10;15;0;100 write_avg_rtt=${WRITE_AVG_RTT}ms;2000;5000;; write_avg_exe=${WRITE_AVG_EXE}ms;2000;5000;;"
                        exit $STATE_OK;;
        esac
fi
